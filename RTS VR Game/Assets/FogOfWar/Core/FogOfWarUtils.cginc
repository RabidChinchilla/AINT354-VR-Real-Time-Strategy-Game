...
float3 GetWorldPosition(v2f i, out float rawdepth)
{
	// for VR
	i.uv = UnityStereoTransformScreenSpaceTex(i.uv);

	float3 viewspacepos = ComputeViewSpacePosition(i.uv, i.interpolatedRay, rawdepth);
...
half4 frag(v2f i) : SV_Target
{
	...
	float rawdepth;
	fogdata.worldPosition = GetWorldPosition(i, rawdepth);
	...
	fogdata.screenPosition = i.uv;
	fogdata.cameraPosition = _CameraWorldPosition;
	fogdata.backgroundColor = tex2D(_MainTex, i.uv);
