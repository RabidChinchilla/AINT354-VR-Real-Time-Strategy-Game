Shader "Hidden/FogOfWarHardware"
{
	Properties
	{
		...
		_LineOfSightDistance("Line Of Sight Distance", float) = 1
	}
	SubShader
	{
		ZTest Always
		...
		Pass
		{
			...
			fixed frag (v2f i) : SV_Target
			{
				...
				fixed lineofsightangle = ClockwiseAngle(fixed2(1, 0), offset) / -(3.14159 * 2) + 0.25;
				fixed lineofsightdist = tex2D(_LineOfSightTex, fixed2(lineofsightangle, 0)).r * _LineOfSightDistance;
				if (maxdist >= lineofsightdist)
					return 1;
#elif LINE_OF_SIGHT_CELLS
				fixed2 uv = (i.uv - _EyePosition.xy - 0.0001) / _LineOfSightDistance + 0.5; // the 0.0001 here is a bit hacky but should work fine on low res maps
