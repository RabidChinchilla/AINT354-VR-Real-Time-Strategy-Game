...
public class SelectUnits : MonoBehaviour {
	...
	void Update () { 
        ...
        if (_player.IsConstructing() || EventSystem.current.IsPointerOverGameObject())
            return;
        ...
        { 
            ...
            if(_selectedUnits.Count > 0)
            {
                ...
                if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit))
                {
                    ...
                    {
                        VehicleController veh = unit.GetComponent<VehicleController>();
                        if (veh != null)
                        {
                            veh.SetDestination(hit.point);
                        }
                    }
